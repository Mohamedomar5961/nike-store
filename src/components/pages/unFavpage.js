...
const UnFavpage = () => {
  ...
  const [checkIfFav, setCheckIfFav] = React.useState(true);
  ...
  const onFavorite = async (id) => {
    ...
    getFavorite().map((items) => {
      return (
        items.id == id ? setCheckIfFav(true) : setCheckIfFav(false),
        favDispatcher()
      );
    });
  };
